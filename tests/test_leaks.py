...
async def test_check_leaks_flags_forbidden_email():
    ...
    email_row = next(r for r in results if r["check"].endswith("README"))
    assert email_row["status"] == "FAIL"
    assert "leaked@gmail.com" in email_row["detail"]
...
async def test_check_leaks_pass_when_clean():
    ...
    results = await check_leaks(token="", repos=[repo])
    # Both rows (email + secrets) should be PASS.
    assert all(r["status"] == "PASS" for r in results), results


@pytest.mark.asyncio
@respx.mock
async def test_check_leaks_flags_github_token_secret():
    repo = "perditioinc/test-repo"
    token_like = "ghp_" + "A" * 40
    respx.get(
        f"https://raw.githubusercontent.com/{repo}/main/README.md"
    ).mock(return_value=httpx.Response(200, text=f"Example: {token_like}"))

    results = await check_leaks(token="", repos=[repo])
    secret_row = next(r for r in results if r["check"].endswith("secrets"))
    assert secret_row["status"] == "FAIL"
    assert "github-token" in secret_row["detail"]


def test_scan_for_secret_patterns_catches_known_forms():
    sample = "Key: AKIA" + "A" * 16 + " and token ghp_" + "B" * 40
    hits = _scan_for_secret_patterns(sample)
    names = {name for name, _ in hits}
    assert "aws-access-key" in names
    assert "github-token" in names


def test_scan_for_secret_patterns_respects_allowlist(monkeypatch):
    sample = "Redacted demo: AKIA" + "A" * 16
    monkeypatch.setenv("AUDIT_SECRET_ALLOWLIST", "AKIAAAAAAAAAAAAAAAAA")
    assert _scan_for_secret_patterns(sample) == []
