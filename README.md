...
- **Repo CI (latest run)** — pass/fail across the active Reporium suite
...
Produces a local `AUDIT_REPORT.md` with the full results table. The
report is generated at runtime and is **not tracked in git** — the
nightly bot regenerates it each run. A `SKIP` status in the Full
Results table means the check couldn't run (usually a missing
credential) rather than passed — the gap is made visible rather than
hidden.
...
The nightly job is GitFlow-compliant: it checks out `dev`, runs the
audit, commits any *tracked* generated changes to `dev` (the report
itself is gitignored and regenerated each run), then publishes by
fast-forwarding `main` to `dev` in the same job (`git push origin
dev:main`). The bot never writes `main` directly outside the
`dev` -> `main` flow, so the two branches cannot diverge.
